...
def _build_translation_prompt(name: str, keywords: list[str], language: str) -> str:
    ...
    return (
        ...
        "2. 优先采用当地商务常用的采购方术语：分销商/经销商/批发商/代理商"
        "（distributor/dealer/wholesaler/agency），并优先给出“产品词 + 采购方术语”的组合；\n"
        "3. 若目标语言是俄语（ru），使用俄语商业术语（如 дистрибьютор、дилер、оптовик、агент），"
        "符合中亚市场习惯，并给出“产品词 + 采购方术语”组合（如 подшипник дистрибьютор）；\n"
